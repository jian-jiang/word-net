...
function process()
{
	...
	echo "$2"
	mysql -h 127.0.0.1 -P 3306 -u ${dbuser} --password=${MYSQLPASSWORD} $4 $3 < $1
}

function dbexists()
{
	mysql -h 127.0.0.1 -P 3306 -u ${dbuser} --password=${MYSQLPASSWORD} -e "\q" ${db} > /dev/null 2> /dev/null
	return $? 
}
...
{
	echo "delete ${db}"
	mysql -h 127.0.0.1 -P 3306 -u ${dbuser} --password=${MYSQLPASSWORD} -e "DROP DATABASE ${db};"
}

function createdb()
{
	echo "create ${db}"
	mysql -h 127.0.0.1 -P 3306 -u ${dbuser} --password=${MYSQLPASSWORD} -e "CREATE DATABASE ${db} DEFAULT CHARACTER SET UTF8;"
}
